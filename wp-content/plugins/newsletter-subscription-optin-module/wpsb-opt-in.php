...
function wpsb_show_form($rtn = 0) {	
	...
	$out = '<form action="#wpsbw" method="post"  id="newsletterForm" >' . "\n";
	$out .= '<input type="text" name="wpsb_email" id="wpsb_email" class="wpsb_form_txt" required placeholder="Digite seu e-mail" />' . "\n";
	if (is_array($wpsb_flds)) {
		foreach ($wpsb_flds as $wpsb_k => $wpsb_v) {
			if (is_numeric($wpsb_k) && $wpsb_v) {
				$out .= ' <input type="text" name="wpsb_fld['. $wpsb_k .']" id="wpsb_fld_'. $wpsb_k .'"  class="wpsb_form_txt"  required placeholder="Digite seu nome"   />' . "\n";
			}
		}
	}
